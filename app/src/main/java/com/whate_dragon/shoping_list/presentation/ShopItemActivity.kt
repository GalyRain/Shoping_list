...
class ShopItemActivity : AppCompatActivity() {
    ...
    private fun observeViewModel() {
        ...
        viewModel.shoutCloseScreen.observe(this) {
            finish()
        }
    }
    ...
    private fun parseIntent() {
        ...
        if (screenMode == MODE_EDIT) {
            if (!intent.hasExtra(EXTRA_SCREEN_ITEM_ID)) {
                throw RuntimeException("Unknown shop item id is absent")
            }
            shopItemId = intent.getIntExtra(EXTRA_SCREEN_ITEM_ID, ShopItem.UNDEFINED_ID)
        }
    }
